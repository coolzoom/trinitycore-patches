...
bool GossipHello_city_conquest_portal(Player* pPlayer, Creature* pCreature)
{
    MainMenu(pPlayer, pCreature);
	return true;
}

bool GossipSelect_city_conquest_portal(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 action)
{
	int id = 0;
	int side = 1;
		if(pPlayer->TeamForRace(pPlayer->getRace())==ALLIANCE)
...
void AddSC_city_conquest_portal()
{
    ...
    newscript->Name = "city_conquest_portal";
    newscript->pGossipHello =  &GossipHello_city_conquest_portal;
    newscript->pGossipSelect = &GossipSelect_city_conquest_portal;
