...
void CityConquestMgr::StartSiege(int id, int faction)
{
	if (CanStartSiege(id, faction))
	{
		...
		else
		{
			...
			WorldDatabase.PExecuteLog("UPDATE city_capture_bosses SET spawn_timer = %u WHERE city_id = %u", t, id);
		}
		...
		WorldDatabase.PExecuteLog("REPLACE INTO city_capture_progress (city_id, time) VALUES (%u, %u)", 
			id, time (NULL));
...
void CityConquestMgr::EndSiege(int id)
{
	...
	if (!city->active)
		return;
	city->active = false;
	WorldDatabase.PExecuteLog("REPLACE INTO city_capture_progress (city_id, time) VALUES (%u, %u, %u)", 
			id, time (NULL));

	WorldDatabase.PExecuteLog("UPDATE city_capture_bosses SET spawn_timer =  1 WHERE city_id = %u", id);
...
void CityConquestMgr::SwitchFaction(int id)
{
	...
	if (city->active)
	{
		...
		if (city->controlled == 0)
			...
			city->controlled = 0;
		WorldDatabase.PExecuteLog("UPDATE city_capture_cities SET controlled = %u WHERE id = %u", city->controlled, city->id);
		EndSiege(id);
	}
}
...
bool CityConquestMgr::CanStartSiege(int id, int faction)
{
	...
	QueryResult_AutoPtr result = WorldDatabase.PQuery("SELECT time FROM city_capture_progress WHERE city_id = %u", id);
	//														  0
	if (result)
	{
...
{
	QueryResult_AutoPtr result = WorldDatabase.PQuery("SELECT id, minlevel, maxlevel, name, controlled, boss_entry_A, boss_entry_H FROM city_capture_cities LEFT JOIN city_capture_bosses ON id = city_id ORDER BY id ASC");
	//														  0   1         2         3     4           5             6
	if (result)
	{
		do
...
void CityConquestMgr::RegisterCreature(Creature * creature, int type)
{
	...
	{
		result 
			= WorldDatabase.PQuery("SELECT city_id, spawner_entry, spawn_timer FROM city_capture_bosses WHERE boss_entry_A = %u OR boss_entry_H = %u OR "
			//                             0        1              2
...
void City::LoadPorts()
{
	QueryResult_AutoPtr result = WorldDatabase.PQuery("SELECT x, y, z, o, map, side FROM city_capture_ports WHERE city_id = %u", id);
	//														  0  1  2  3  4    5
	if (result)
	{
		do
