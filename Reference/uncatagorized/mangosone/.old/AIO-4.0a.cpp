...
// Gem ------------------------
#define Gembuy "I want to buy a %Servername% Gem."
#define Gemchange "I want to change a %Gemname% to a %Gemname2%."

// mails
#define moneymail 60003

// items
#define Gem1 99999 // Gem1 entry number (swap). 
#define Gem2 99998 // Gem2 entry number (receive).
#define vasarolhatoGem 100000 // Available as Gem -.-
...
bool GossipHello_all_npc(Player *player, Creature *_Creature)
 
{
	_Creature->MonsterSay(welcome, NULL);
	...
	//player->ADD_GOSSIP_ITEM(0, "[Menu]Gem Buy", GOSSIP_SENDER_MAIN, 107); // Not configured
	//player->ADD_GOSSIP_ITEM(0, "[Menu]Gem Change", GOSSIP_SENDER_MAIN, 108); // Not Configured
	...
	player->ADD_GOSSIP_ITEM(0, "[Menu]Buffs", GOSSIP_SENDER_MAIN, 125);
		...
		{
		_Creature->MonsterSay(yesvip, NULL);
		...
		} else { _Creature->MonsterSay(notvip, NULL); }
...
if (player->isInCombat())
{
	...
	case 102: // Skill maxer
		...
		_Creature->MonsterSay(skillsmaxed, NULL);
		break;
	...
	case 106: // Announcer Part
		...
		break;
	case 107: // Gem -> BUY
		player->ADD_GOSSIP_ITEM(0, Gembuy, GOSSIP_SENDER_MAIN, 1200);
        player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE,_Creature->GetObjectGuid());
		break;

	case 108: // Gem ->Change
		player->ADD_GOSSIP_ITEM(0, Gemchange, GOSSIP_SENDER_MAIN, 1300);
        player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE,_Creature->GetObjectGuid());
		break;
	...
	case 111: // VOte
        _Creature->MonsterSay(votewhisper, NULL);
	...
	case 4000:
        ...
        player->CLOSE_GOSSIP_MENU();
		_Creature->MonsterSay(('%s', player->GetSession()->GetAccountId()), NULL);
        //LoginDatabase.PExecute("INSERT INTO `lagreports` (`account`) VALUES ('%s'", player->GetSession()->GetAccountId());
        _Creature->MonsterSay("Lagg report done.", NULL);
        break;
        ...
        player->CLOSE_GOSSIP_MENU();
        _Creature->MonsterSay("Donations can be done on www.flaminglegion.net", NULL);
        break;

		case 1701: // INGAME HELP
        player->CLOSE_GOSSIP_MENU();
        _Creature->MonsterSay("Contact a GM for help", NULL);
        break;

		case 1702: // Normal Help
        player->CLOSE_GOSSIP_MENU();
        _Creature->MonsterSay("Helpful information can be found on www.flaminglegion.net", NULL);
        break;
        ...
        _Creature->MonsterSay("Check your mailbox.", NULL);
        break;
...
// Zones
	...
	/*        player->ADD_GOSSIP_ITEM(0, "WSG 100%", GOSSIP_SENDER_MAIN, 1400);
		...
		case 1201: // Gem change
        if(player->HasItemCount(Gem1, 1, false))
        {
        player->ItemRemovedQuestCheck(Gem1, 1);
        player->SendNewItem(Gem2, 1,false,true);
        } else {
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "You don't have enough gems.", GOSSIP_SENDER_MAIN, 12345);
        break;
        }

		case 1200:
        player->ADD_GOSSIP_ITEM_EXTENDED(0, Gembuy, GOSSIP_SENDER_MAIN, 2000, "Are you sure you want to buy a gem?", -10000, 0);
        break;
		case 2000:
        player->AddItem(vasarolhatoGem, 1);
        break;*/
		...
		*/
        ...
        player->CLOSE_GOSSIP_MENU();
        player->SendTalentWipeConfirm(_Creature->GetObjectGuid());
        break;

        case 120: //Sickness leszedése
        if(!player->HasAura(SPELL_ID_PASSIVE_RESURRECTION_SICKNESS))
        {
        	_Creature->MonsterSay(nosickness, NULL);
        }
        else
        {
...
bool GossipSelect_all_npc(Player* player, Creature* _Creature, uint32 sender, uint32 action)
{
	// Main menu
...
void AddSC_all_npc()
{
    ...
    newscript->Name="all_npc";
    newscript->pGossipHello =  &GossipHello_all_npc;
    newscript->pGossipSelect = &GossipSelect_all_npc;
