...
void teleportPlayerToGuildHouse(Player *player, Creature *_creature)
{
    ...
    {
        //if player has no guild
        _creature->MonsterSay(MSG_NOTINGUILD, NULL);
        return;
    }
    ...
    {
        //if player in combat
        _creature->MonsterSay(MSG_INCOMBAT, LANG_UNIVERSAL, NULL);
        return;
    }
    ...
    if (getGuildHouseCoords(player->GetGuildId(), x, y, z, map))
    {
        ...
        _creature->MonsterSay(MSG_NOGUILDHOUSE, NULL);
...
bool showBuyList(Player *player, Creature *_creature, uint32 showFromId = 0)
{
    ...
    } else
    {
        if (showFromId == 0)
        {
            ...
            _creature->MonsterSay(MSG_NOFREEGH, NULL);
...
void buyGuildhouse(Player *player, Creature *_creature, uint32 guildhouseId)
{
    ...
    if (result)
    {
        _creature->MonsterSay(MSG_GHOCCUPIED, NULL);
        return;
    }

    //update DB
    result = WorldDatabase.PQuery("UPDATE `guild_houses` SET `guildId` = %u WHERE `id` = %u",
        player->GetGuildId(), guildhouseId);
    
    player->ModifyMoney(-buyprice);
    _creature->MonsterSay(MSG_CONGRATULATIONS, LANG_UNIVERSAL, NULL);
