...
bool GossipHello_npc_all(Player *player, Creature *_Creature)
 
{
	_Creature->MonsterSay(welcome, player->GetObjectGuid());
	...
	player->ADD_GOSSIP_ITEM(0, "[Menu]Buffs", GOSSIP_SENDER_MAIN, 125);
		...
		{
		_Creature->MonsterSay(yesvip, player->GetObjectGuid());
		...
		} else { _Creature->MonsterSay(notvip, player->GetObjectGuid()); }
...
void SendDefaultMenu_npc_all(Player *player, Creature *_Creature, uint32 action)
{
...
if (player->isInCombat())
{
	...
	case 102: // Skill maxer
		...
		_Creature->MonsterSay(skillsmaxed, player->GetObjectGuid());
		break;
	...
	case 111: // VOte
        _Creature->MonsterSay(votewhisper, player->GetObjectGuid());
	...
	case 4000:
        ...
        player->CLOSE_GOSSIP_MENU();
		_Creature->MonsterSay(('%s', player->GetSession()->GetAccountId()), player->GetObjectGuid());
        //LoginDatabase.PExecute("INSERT INTO `lagreports` (`account`) VALUES ('%s'", player->GetSession()->GetAccountId());
        _Creature->MonsterSay("Lagg report done.", player->GetObjectGuid());
        break;
        ...
        player->CLOSE_GOSSIP_MENU();
        _Creature->MonsterSay("Donations can be done on www.flaminglegion.net", player->GetObjectGuid());
        break;

		case 1701: // INGAME HELP
        player->CLOSE_GOSSIP_MENU();
        _Creature->MonsterSay("Contact a GM for help", player->GetObjectGuid());
        break;

		case 1702: // Normal Help
        player->CLOSE_GOSSIP_MENU();
        _Creature->MonsterSay("Helpful information can be found on www.flaminglegion.net", player->GetObjectGuid());
        break;
        ...
        _Creature->MonsterSay("Check your mailbox.", player->GetObjectGuid());
        break;
...
// Zones
	...
	/*        player->ADD_GOSSIP_ITEM(0, "WSG 100%", GOSSIP_SENDER_MAIN, 1400);
		...
		*/
        ...
        {
        	_Creature->MonsterSay(nosickness, player->GetObjectGuid());
        }
        else
        {
...
bool GossipSelect_npc_all(Player* player, Creature* _Creature, uint32 sender, uint32 action)
{
	// Main menu
	player->PlayerTalkClass->ClearMenus();
	if (sender == GOSSIP_SENDER_MAIN)
	SendDefaultMenu_npc_all(player, _Creature, action);
...
void AddSC_npc_all()
{
    ...
    pNewScript->Name = "npc_all";
    pNewScript->pGossipHello = &GossipHello_npc_all;
    pNewScript->pGossipSelect = &GossipSelect_npc_all;
