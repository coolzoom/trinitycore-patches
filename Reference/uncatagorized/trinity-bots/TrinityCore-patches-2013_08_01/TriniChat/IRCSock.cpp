...
bool IRCClient::InitSock()
{
    ...
    {
        TC_LOG_ERROR("IRC", "IRC Error: Winsock Initialization Error");
        return false;
    }
    #endif
    if ((sIRC.SOCKET = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) == -1)
    {
        TC_LOG_ERROR("IRC", "IRC Error: Socket Error");
        return false;
    }
    int on = 1;
    if (setsockopt (sIRC.SOCKET, SOL_SOCKET, SO_REUSEADDR, (const char*) &on, sizeof (on)) == -1)
    {
        TC_LOG_ERROR("IRC", "IRC Error: Invalid Socket");
        return false;
    }
    #ifdef _WIN32
    u_long iMode = 0;
...
bool IRCClient::Connect(const char *cHost, int nPort)
{
    ...
    {
        TC_LOG_ERROR("IRC", "IRCLIENT: Could not resolve host: %s", cHost);
        return false;
    }
    ...
    {
        TC_LOG_ERROR("IRC", "IRCLIENT: Cannot connect to %s", cHost);
        return false;
    }
    //FD_ZERO(&sIRC.sfdset);
...
bool IRCClient::SendData(const char *data)
{
    if (sIRC.Connected)
    {
        ...
        {
            TC_LOG_ERROR("IRC", "IRC Error: Socket Receieve ** \n");
            //Disconnect();
            return false;
        }
    }
...
//    wchar_t bufferdata;
    ...
    if (nBytesRecv == -1)
    {
        TC_LOG_ERROR("IRC", "Connection lost.");
        sIRC.Connected = false;
    }
    else
    {
        if (-1 == nBytesRecv)
        {
            TC_LOG_ERROR("IRC", "Error occurred while receiving from socket.");
        }
        else
        {
            std::string reply;
