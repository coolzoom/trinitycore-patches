...
void IRCClient::run()
{
    ...
    TC_LOG_ERROR("IRC", "\n%s\n%s\n%s\n%s",
        ...
        "***************************************");
    TC_LOG_ERROR("IRC", "****** TriniChat: %s ********", sIRC._Mver.c_str());
    int cCount = 1;
    // Clean Up MySQL Tables
    TC_LOG_ERROR("IRC", "*** TriniChat: Cleaning Up Inchan Table*");
    ...
    while (sIRC.Active && !World::IsStopped())
    {
        ...
        if (this->InitSock())
        {
            ...
            TC_LOG_ERROR("IRC", "*** TriniChat: Connecting to %s Try # %d ******", sIRC._Host.c_str(), cCount);
           if (this->Connect(sIRC._Host.c_str(), sIRC._Port))
           {
               // On connection success reset the connection counter
               cCount = 0;
               TC_LOG_ERROR("IRC", "*** TriniChat: Connected And Logging In*");
                // Login to the IRC server
                if (this->Login(sIRC._Nick, sIRC._User, sIRC._Pass))
                {
                    TC_LOG_ERROR("IRC", "*** TriniChat: Logged In And Running!! *");
                ...
                TC_LOG_ERROR("IRC", "*** TriniChat: Connection To IRC Server Lost! ***");
            }
        ...
        else
        {
            ...
            sIRC.Active = false;
            TC_LOG_ERROR("IRC", "** TriniChat: Could not initialize socket");
        }
    }
