...
{
if (!pPlayer->HasItemCount((sConfigMgr->GetIntDefault("LevelNPC.ItemEntryNum",1)), 0))
{
    pPlayer->CLOSE_GOSSIP_MENU();
    pCreature->MonsterWhisper("You need a Max Level Token. You can get this on the website.", pPlayer->GetGUID());
	return;
}
}
...
case 1000: //Leveling
	if(EnableLevel80 && pPlayer->getLevel() < 80)
	    {
		pPlayer->ADD_GOSSIP_ITEM( 7, "(1 Max level Token) Instant 80 ->"         , GOSSIP_SENDER_MAIN, 1001);
/*	if(EnableLevel100 && pPlayer->getLevel() < 100 && pPlayer->getLevel() >= 80)
	...
	pPlayer->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE,pCreature->GetGUID());
	    ...
	    {
		pPlayer->CLOSE_GOSSIP_MENU();
		pCreature->MonsterWhisper("You are already at the current max level.", pPlayer->GetGUID());
...
/*case 1002: // Leveling
	...
	if(sConfigMgr->GetBoolDefault("LevelNPC.UseTokens", true))
	pPlayer->DestroyItemCount(99995, 2, true);
	else
	pPlayer->ModifyMoney(-(sConfigMgr->GetIntDefault("LevelNPC.SkillGoldCost",0)));
	pPlayer->CLOSE_GOSSIP_MENU();
break;
case 1003: // Leveling
	...
	if(sConfigMgr->GetBoolDefault("LevelNPC.UseTokens", true))
	pPlayer->DestroyItemCount(99995, 5, true);
	else
	pPlayer->ModifyMoney(-(sConfigMgr->GetIntDefault("LevelNPC.SkillGoldCost",0)));
	pPlayer->CLOSE_GOSSIP_MENU();
break;
case 1004: // Leveling
	...
	if(sConfigMgr->GetBoolDefault("LevelNPC.UseTokens", true))
	pPlayer->DestroyItemCount(99995, 10, true);
	else
	pPlayer->ModifyMoney(-(sConfigMgr->GetIntDefault("LevelNPC.SkillGoldCost",0)));
	pPlayer->CLOSE_GOSSIP_MENU();
break;
case 1005: // Leveling
	...
	if(sConfigMgr->GetBoolDefault("LevelNPC.UseTokens", true))
	pPlayer->DestroyItemCount(99995, 20, true);
	else
	pPlayer->ModifyMoney(-(sConfigMgr->GetIntDefault("LevelNPC.SkillGoldCost",0)));
	pPlayer->CLOSE_GOSSIP_MENU();
break;*/
