...
/*
        if (go) // object
        {
            ...
            {
                UseItem(m_bot->GetItemByGuid(reqItem), TARGET_FLAG_OBJECT, m_lootCurrent);
                m_lootCurrent = ObjectGuid();
                return;
            }
            else
                ...
                switch (skillId)
                {
                    ...
                    case SKILL_SKINNING:
                        if (c && HasCollectFlag(COLLECT_FLAG_SKIN) &&
                            HasTool(TC_SKINNING_SKIFE) && CastSpell(SKINNING, *c))
                            return;
                        else
                            skillFailed = true;
                        break;
        ...
        if (subcommand == "train" || subcommand == "learn")
        {
            ...
            if (subcommand == "learn")
            {
                ...
                for (std::list<uint32>::iterator it = m_spellsToLearn.begin(); it != m_spellsToLearn.end(); it++)
                {
                    ...
                    if (!trainer_spell || !trainer_spell->spell)
                        continue;
            ...
            if (subcommand == "train")
            {
                ...
                for (TrainerSpellMap::const_iterator itr =  trainer_spells->spellList.begin(); itr !=  trainer_spells->spellList.end(); ++itr)
                {
                    ...
                    if (!tSpell->spell && !m_bot->IsSpellFitByClassAndRace(tSpell->spell))
                        continue;

                    if  (sSpellMgr.IsPrimaryProfessionFirstRankSpell(tSpell->spell) && m_bot->HasSpell(tSpell->spell))
                        continue;
