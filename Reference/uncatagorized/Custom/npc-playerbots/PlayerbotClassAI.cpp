...
Unit *PlayerbotClassAI::FindMainTankInRaid(Player *gPlayer)
{
    ...
    {
        QueryResult_AutoPtr result = CharacterDatabase.PQuery("SELECT memberGuid FROM group_member WHERE memberFlags='%u' AND leaderGuid = '%u'",MEMBER_FLAG_MAINTANK, pGroup->GetLeaderGUID());
        if(result)
        {    
...
Unit *PlayerbotClassAI::FindMainAssistInRaid(Player *gPlayer)
{
    ...
    if (pGroup->isRaidGroup())
    {
        QueryResult_AutoPtr result = CharacterDatabase.PQuery("SELECT memberGuid FROM group_member WHERE memberFlags='%u' AND leaderGuid = '%u'",MEMBER_FLAG_MAINASSIST, pGroup->GetLeaderGUID());
          if(result)
        {    
            uint64 pGuid = MAKE_NEW_GUID(result->Fetch()->GetInt32(),0,HIGHGUID_PLAYER);
            pPlayer = objmgr.GetPlayer(pGuid);
