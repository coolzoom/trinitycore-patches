    ...
    for (itr = m_QueuedGroups[queue_id].begin(); itr != m_QueuedGroups[queue_id].end(); itr = next)
    {
        ...
        if (ginfo->IsInvitedToBGInstanceGUID == bgInstanceId)
        {
            ...
            for (uint32 i = 0; i < to_remove; ++i)
            {
                ...
                {
                    TC_LOG_ERROR("bg.battleground", "Empty Players in ginfo, this should never happen!");
                    return;
                }
                ...
                if (!plr)
                {
                    TC_LOG_ERROR("bg.battleground", "Player offline when trying to remove from GroupQueueInfo, this should never happen.");
                    continue;
                }
...
void BattlegroundQueue::Update(uint32 bgTypeId, uint32 queue_id, uint8 arenatype, bool isRated, uint32 arenaRating)
{
    if (queue_id >= MAX_BATTLEGROUND_QUEUES)
    {
        ...
        TC_LOG_ERROR("bg.battleground", "BattlegroundQueue::Update() called for invalid queue type - this can cause crash, pls report problem, if this is the last line of error log before crash");
        return;
    }
...
void BattlegroundMgr::BuildPvpLogDataPacket(WorldPacket *data, Battleground *bg)
{
    ...
    *data << uint32(scoreCount);                            // placeholder
    ...
    for (Battleground::BattlegroundScoreMap::const_iterator itr = itr2; itr != bg->GetPlayerScoresEnd();)
    {
        ...
        {
            TC_LOG_ERROR("bg.battleground", "Player " UI64FMTD " has scoreboard entry for battleground %u but is not in battleground!", itr->first, bg->GetTypeID());
            continue;
        }
        ...
        *data << uint32(itr2->second->HealingDone);             // healing done
        ...
        {
            TC_LOG_ERROR("bg.battleground", "Battleground %u scoreboard has more entries (%u) than allowed players in this bg (%u)", bg->GetTypeID(), bg->GetPlayerScoresSize(), bg->GetMaxPlayers());
            break;
        }
    }
...
void BattlegroundMgr::CreateInitialBattlegrounds()
{
    ...
    {
        sLog.outString();
        TC_LOG_ERROR("bg.battleground", ">> Loaded 0 battlegrounds. DB table battleground_template is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("bg.battleground", "Table battleground_template for id %u has invalid WorldSafeLocs.dbc id %u in field AllianceStartLoc. BG not created.",bgTypeID,start1);
            continue;
        }
        ...
        {
            TC_LOG_ERROR("bg.battleground", "Table battleground_template for id %u has invalid WorldSafeLocs.dbc id %u in field HordeStartLoc. BG not created.",bgTypeID,start2);
            continue;
        }

        //TC_LOG_ERROR("bg.battleground", "Creating battleground %s, %u-%u", bl->name[sWorld.GetDBClang()], MinLvl, MaxLvl);
...
void BattlegroundMgr::SendToBattleground(Player *pl, uint32 instanceId)
{
    ...
    if (bg)
    {
        ...
        TC_LOG_ERROR("bg.battleground", "BATTLEGROUND: Sending %s to map %u, X %f, Y %f, Z %f, O %f", pl->GetName(), mapid, x, y, z, O);
        pl->TeleportTo(mapid, x, y, z, O);
    }
    else
    {
        TC_LOG_ERROR("bg.battleground", "player %u trying to port to non-existent bg instance %u",pl->GetGUIDLow(), instanceId);
    }
}
