...
void BattlegroundAV::HandleKillUnit(Creature *unit, Player *killer)
{
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_CAPTAIN][0])
    {
        ...
        Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
        if (creature)
            YellToAll(creature,GetOregonString(LANG_BG_AV_H_CAPTAIN_DEAD),LANG_UNIVERSAL);
    }
...
void BattlegroundAV::HandleQuestComplete(uint32 questid, Player *player)
{
    ...
    TC_LOG_DEBUG("bg.battleground", "BattlegroundAV: Quest %i completed",questid);
    switch(questid)
    {
        ...
        case BG_AV_QUEST_H_SCRAPS2:
            ...
            {
                TC_LOG_DEBUG("bg.battleground", "BattlegroundAV: Quest %i completed starting with unit upgrading..",questid);
        ...
        case BG_AV_QUEST_H_COMMANDER1:
            ...
            if (m_Team_QuestStatus[team][1] == 30)
                TC_LOG_DEBUG("bg.battleground", "BattlegroundAV: Quest %i completed (need to implement some events here",questid);
            break;
        ...
        case BG_AV_QUEST_H_NEAR_MINE:
            ...
            if (m_Team_QuestStatus[team][5] == 28)
            {
                ...
                if (m_Team_QuestStatus[team][6] == 7)
                    TC_LOG_DEBUG("bg.battleground", "BattlegroundAV: Quest %i completed (need to implement some events here - ground assault ready",questid);
            }
            break;
        ...
        case BG_AV_QUEST_H_RIDER_HIDE:
            ...
            if (m_Team_QuestStatus[team][7] == 25)
            {
                ...
                if (m_Team_QuestStatus[team][8] == 25)
                    TC_LOG_DEBUG("bg.battleground", "BattlegroundAV: Quest %i completed (need to implement some events here - rider assault ready",questid);
            }
            break;
        ...
        case BG_AV_QUEST_H_RIDER_TAME:
            ...
            break;
        default:
            TC_LOG_DEBUG("bg.battleground", "BattlegroundAV: Quest %i completed but is not interesting at all",questid);
            return;
            break;
    }
}
...
void BattlegroundAV::HandleAreaTrigger(Player *Source, uint32 Trigger)
{
    ...
    switch(Trigger)
    {
        ...
        case 3331:
            ...
            break;
        default:
            TC_LOG_DEBUG("bg.battleground", "BattlegroundAV: WARNING: Unhandled AreaTrigger in Battleground: %u", Trigger);
...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    if (IsTower(node))
    {
        ...
        if (m_BgCreatures[AV_CPLACE_A_MARSHAL_SOUTH + tmp])
            ...
            TC_LOG_ERROR("bg.battleground", "BattlegroundAV:: playerdestroyedpoint: marshal %i doesn't exist",AV_CPLACE_A_MARSHAL_SOUTH + tmp);
        //spawn destroyed aura
        for (uint8 i=0; i <= 9; i++)
...
void BattlegroundAV::EventPlayerClickedOnFlag(Player *source, GameObject* target_obj)
{
    ...
    TC_LOG_DEBUG("bg.battleground", "BattlegroundAV: using gameobject %i with type %i",target_obj->GetEntry(),object);
    if (object < 0)
        return;
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* player, uint32 object)
{
    ...
    {
        TC_LOG_ERROR("bg.battleground", "BattlegroundAV: player defends point which doesn't belong to his team %i", node);
        return;
    }
...
uint8 BattlegroundAV::GetWorldStateType(uint8 state, uint16 team) //this is used for node worldstates and returns values which fit good into the worldstatesarray
{
    ...
    TC_LOG_ERROR("bg.battleground", "BattlegroundAV:: should update a strange worldstate state:%i team:%i",state,team);
...
bool BattlegroundAV::SetupBattleground()
{
    ...
    //creatures
    TC_LOG_DEBUG("bg.battleground", "BattlegroundAV: start poputlating nodes");
    for (BG_AV_Nodes i= BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX; ++i)
    {
        if (m_Nodes[i].Owner)
            PopulateNode(i);
    ...
    TC_LOG_DEBUG("bg.battleground", "BattlegroundAV: start spawning static creatures");
    for (i=0; i < AV_STATICCPLACE_MAX; i++)
        AddAVCreature(0,i+AV_CPLACE_MAX);
    //mainspiritguides:
    TC_LOG_DEBUG("bg.battleground", "BattlegroundAV: start spawning spiritguides creatures");
    ...
    TC_LOG_DEBUG("bg.battleground", "BattlegroundAV: start spawning marshal creatures");
...
const char* BattlegroundAV::GetNodeName(BG_AV_Nodes node)
{
    switch (node)
    {
        case BG_AV_NODES_FIRSTAID_STATION:  return GetOregonString(LANG_BG_AV_NODE_GRAVE_STORM_AID);
        case BG_AV_NODES_DUNBALDAR_SOUTH:   return GetOregonString(LANG_BG_AV_NODE_TOWER_DUN_S);
        case BG_AV_NODES_DUNBALDAR_NORTH:   return GetOregonString(LANG_BG_AV_NODE_TOWER_DUN_N);
        case BG_AV_NODES_STORMPIKE_GRAVE:   return GetOregonString(LANG_BG_AV_NODE_GRAVE_STORMPIKE);
        case BG_AV_NODES_ICEWING_BUNKER:    return GetOregonString(LANG_BG_AV_NODE_TOWER_ICEWING);
        case BG_AV_NODES_STONEHEART_GRAVE:  return GetOregonString(LANG_BG_AV_NODE_GRAVE_STONE);
        case BG_AV_NODES_STONEHEART_BUNKER: return GetOregonString(LANG_BG_AV_NODE_TOWER_STONE);
        case BG_AV_NODES_SNOWFALL_GRAVE:    return GetOregonString(LANG_BG_AV_NODE_GRAVE_SNOW);
        case BG_AV_NODES_ICEBLOOD_TOWER:    return GetOregonString(LANG_BG_AV_NODE_TOWER_ICE);
        case BG_AV_NODES_ICEBLOOD_GRAVE:    return GetOregonString(LANG_BG_AV_NODE_GRAVE_ICE);
        case BG_AV_NODES_TOWER_POINT:       return GetOregonString(LANG_BG_AV_NODE_TOWER_POINT);
        case BG_AV_NODES_FROSTWOLF_GRAVE:   return GetOregonString(LANG_BG_AV_NODE_GRAVE_FROST);
        case BG_AV_NODES_FROSTWOLF_ETOWER:  return GetOregonString(LANG_BG_AV_NODE_TOWER_FROST_E);
