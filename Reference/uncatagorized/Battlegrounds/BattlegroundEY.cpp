    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            while (j < m_PlayersNearPoint[EY_POINTS_MAX].size())
            {
                ...
                if (!plr)
                {
                    TC_LOG_ERROR("bg.battleground", "BattlegroundEY: Player (GUID: %u) not found!", GUID_LOPART(m_PlayersNearPoint[EY_POINTS_MAX][j]));
                    ++j;
                    continue;
                }
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            while (j < m_PlayersNearPoint[i].size())
            {
                ...
                if (!plr)
                {
                    TC_LOG_ERROR("bg.battleground", "BattlegroundEY: Player (GUID: %u) not found!", GUID_LOPART(m_PlayersNearPoint[i][j]));
...
bool BattlegroundEY::SetupBattleground()
{
    ...
    {
        TC_LOG_ERROR("bg.battleground", "BatteGroundEY: Failed to spawn some object Battleground not created!");
        return false;
    }
    ...
    {
        TC_LOG_ERROR("bg.battleground", "BatteGroundEY: Failed to spawn spirit guide! Battleground not created!");
        return false;
    }
