...
void Battleground::SendPacketToAll(WorldPacket *packet)
{
    for (std::map<uint64, BattlegroundPlayer>::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (plr)
            ...
            TC_LOG_ERROR("bg.battleground", "Battleground: Player (GUID: %u) not found!", GUID_LOPART(itr->first));
    }
}
...
void Battleground::Reset()
{
    ...
    if (m_InvitedAlliance > 0 || m_InvitedHorde > 0)
        TC_LOG_ERROR("bg.battleground", "Battleground system: bad counter, m_InvitedAlliance: %d, m_InvitedHorde: %d", m_InvitedAlliance, m_InvitedHorde);
...
void Battleground::UpdatePlayerScore(Player *Source, uint32 type, uint32 value)
{
    ...
    switch(type)
    {
        ...
        case SCORE_HEALING_DONE:                            // Healing Done
            ...
            break;
        default:
            TC_LOG_ERROR("bg.battleground", "Battleground: Unknown player score type %u", type);
            break;
    }
}
...
bool Battleground::AddObject(uint32 type, uint32 entry, float x, float y, float z, float o, float rotation0, float rotation1, float rotation2, float rotation3, uint32 /*respawnTime*/)
{
    ...
    {
        TC_LOG_ERROR("bg.battleground", "Gameobject template %u not found in database! Battleground not created!", entry);
        TC_LOG_ERROR("bg.battleground", "Cannot create gameobject template %u! Battleground not created!", entry);
        delete go;
        return false;
    }
/*
...
void Battleground::DoorOpen(uint32 type)
{
    ...
    {
        TC_LOG_ERROR("bg.battleground", "Battleground: Door object not found! - doors will be closed.");
    }
}
...
GameObject* Battleground::GetBGObject(uint32 type)
{
    GameObject *obj = GetBgMap()->GetGameObject(m_BgObjects[type]);
    if (!obj)
        TC_LOG_ERROR("bg.battleground", "couldn't get gameobject %i",type);
    return obj;
}
...
Creature* Battleground::GetBGCreature(uint32 type)
{
    ...
    if (!creature)
        TC_LOG_ERROR("bg.battleground", "couldn't get creature %i",type);
    return creature;
}
...
Creature* Battleground::AddCreature(uint32 entry, uint32 type, uint32 teamval, float x, float y, float z, float o, uint32 /*respawntime*/)
{
    ...
    {
        TC_LOG_ERROR("bg.battleground", "Can't create creature entry: %u",entry);
        delete pCreature;
        return NULL;
    }
...
bool Battleground::DelCreature(uint32 type)
{
    ...
    if (!cr)
    {
        TC_LOG_ERROR("bg.battleground", "Can't find creature guid: %u",GUID_LOPART(m_BgCreatures[type]));
        return false;
    }
...
bool Battleground::DelObject(uint32 type)
{
    ...
    if (!obj)
    {
        TC_LOG_ERROR("bg.battleground", "Can't find gobject guid: %u",GUID_LOPART(m_BgObjects[type]));
        return false;
    }
...
void Battleground::HandleTriggerBuff(uint64 const& go_guid)
{
    ...
    if (index < 0)
    {
        TC_LOG_ERROR("bg.battleground", "Battleground (Type: %u) has buff gameobject (Guid: %u Entry: %u Type:%u) but it hasn't that object in its internal data",GetTypeID(),GUID_LOPART(go_guid),obj->GetEntry(),obj->GetGoType());
        return;
    }
...
void Battleground::PlayerRelogin(uint64 guid)
{
    ...
    if (!plr)
    {
        TC_LOG_ERROR("bg.battleground", "Battleground: Player (GUID: %u) not found!", GUID_LOPART(guid));
        return;
    }
