			...
			switch(action)
			{
			   case GOSSIP_ACTION_INFO_DEF+1:
				   for(map<uint64, BountyInfo>::const_iterator i = Bounty.begin(); i != Bounty.end(); ++i)
				   {
					   ...
					   if(i->second.hunted == player->GetObjectGuid())
					   {
					   ...
					   if(i->second.hunted == hunted->GetObjectGuid())
					   {
				   ...
				   Bounty[hunted->GetObjectGuid()].hunted = hunted->GetObjectGuid();
				   Bounty[hunted->GetObjectGuid()].hunter = player->GetObjectGuid();
				   Bounty[hunted->GetObjectGuid()].gold = BOUNTY_AMOUNT_GOLD;
				   Bounty[hunted->GetObjectGuid()].name = name.c_str();
				   Bounty[hunted->GetObjectGuid()].bounty = player->GetName();
				   ChatHandler(player).PSendSysMessage("|cffFF0000Bounty was placed on %s!|r", name.c_str());
				   player->Whisper("I placed a Bounty on you!", LANG_UNIVERSAL, hunted->GetObjectGuid());
...
class bounty_kills : public PlayerScript
{
   public:
	   ...
	   {
		   if(killer->GetObjectGuid() == victim->GetObjectGuid() || Bounty.empty())
			   return;

		   for(map<uint64, BountyInfo>::const_iterator i = Bounty.begin(); i != Bounty.end(); ++i)
		   {
			   if(i->second.hunted == victim->GetObjectGuid())
			   {
				   killer->ModifyMoney(Bounty[victim->GetObjectGuid()].gold);
				   ChatHandler(killer).PSendSysMessage("Added %u gold for your kill!", Bounty[victim->GetObjectGuid()].gold);
				   Bounty.erase(victim->GetObjectGuid());
